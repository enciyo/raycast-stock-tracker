 ...
 */
function calculateRelevanceScore<T extends StockItem | WatchlistItem>(
  stock: T,
  query: string
): number {
 ...
 */
export function filterStocksByQuery<T extends StockItem | WatchlistItem>(
  stocks: T[],
  query: string
): T[] {
  if (!query.trim()) {
  ...
  return stocks.filter((stock) => {
    ...
    return words.every(
      (word) => name.includes(word) || symbol.includes(word) || symbolOnly.includes(word)
    );
  });
}
 ...
 */
export function sortStocksByRelevance<T extends StockItem | WatchlistItem>(
  stocks: T[],
  query: string
): T[] {
