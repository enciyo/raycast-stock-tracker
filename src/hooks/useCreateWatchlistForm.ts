...
export function useCreateWatchlistForm({
  onSuccess,
  stockToAdd,
  mode = "create",
}: UseCreateWatchlistFormProps) {
  const { t } = useI18n();
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (mode === "createAndAdd" && stockToAdd) {
        ...
        showSuccessToast(
          t.watchlist.addedTitle,
          t.watchlist.createdMessage(stockToAdd.name ?? stockToAdd.symbol)
        );
      } else {
      ...
      handleError(error, {
        ...
        message:
          mode === "createAndAdd" ? t.watchlist.createErrorMessage : t.portfolio.createErrorMessage,
        translations: t,
      });
    }
  };
  ...
  const submitTitle =
    mode === "createAndAdd" ? t.watchlist.createAndAddTitle : t.portfolio.createTitle;
